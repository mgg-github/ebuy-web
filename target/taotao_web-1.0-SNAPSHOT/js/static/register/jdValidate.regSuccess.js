...
(function () {
    ...
    reviseNickname.find('.j_save').click(function () {
        ...
        $.getJSON("../validate/newNickname?nickname=" + escape(username) + "&k=" + getKey() + "&r=" + Math.random(), function (date) {
            ...
            if (date.success == -4) {
                window.location.href = "http://reg.taotao.com/reg/expire";
                return;
            }
        })
    });
    ...
    $('#emailStr').focus(function () {
        ...
        $("#email_error").html("");
        $("#email_focus").html("完成验证后，您可以用该邮箱登录淘淘，找回密码。");
    });
    ...
    $('#moblie').bind('focus', function () {
        ...
        $("#smsFocusDiv").removeClass().addClass("item");
        $("#smsFocusMessage").removeClass().addClass("sms-tips mobileFocus").text("完成验证后，您可以用该手机号登录淘淘，找回密码。");
    });
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result) {
                    ...
                    if (obj.rs == -5) {
                        window.location.href = "http://reg.taotao.com/reg/expire";
                        //mobileCodeError("链接已失效，您可以前往<a href='http://safe.taotao.com/user/paymentpassword/safetyCenter.action'>安全中心</a>继续验证。");
                    }
                    if (obj.rs == -7) {
                        mobileCodeError("您已验证过手机，请到<a href='http://safe.taotao.com/user/paymentpassword/safetyCenter.action' class='emreg-nickname'>账户安全</a>里查看。");
                    }
                    if (obj.info) {
    ...
    $('#toValidate').click(function () {
        ...
        $.getJSON("../reg/validateMobile?" + data, function (result) {
                if (result.success == 1) {
                    window.location.href = "http://reg.taotao.com/reg/best?ret=" + result.ret;
                    return;
                }
                if (result.success == -1) {
                ...
                if (result.success == -5) {
                    clientError("您已验证过手机，请到<a href='http://safe.taotao.com/user/paymentpassword/safetyCenter.action' class='emreg-nickname'>账户安全</a>里查看。");
                    return;
                }
                if (result.success == -7) {
                    window.location.href = "http://reg.taotao.com/reg/expire";
...
function reSendEmail(email, key) {
    ...
    $.getJSON("../notifyuser/email?email=" + (email) + "&k=" + key + "&state=" + unbind+ "&r=" + Math.random(), function (result) {
        ...
        if (result.success == -4) {
            $('#reSendEmailSuccess').removeClass().addClass('error').empty().html('该邮箱已注册过淘淘');
            $('#reSendEmailSuccess').show();
            return;
        }
