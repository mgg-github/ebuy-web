...
$.extend(validateFunction, {
    ...
    FORM_validate: function () {
        if($("#chkOpenCtrl").attr("checked")==true){
            var srcValue = $("#TT_Verification1").attr("src");
            if (!srcValue) {
                srcValue = $("#TT_Verification1").attr("src2");
            }
...
//                    }
//                    if(obj.venture){
//                    	window.location = "http://safe.taotao.com/dangerousVerify/index.action?username=" + obj.venture + "&ReturnUrl="+ encodeURI(obj.ventureRet) + "&t=" + new Date().getTime();
//                    	return;
//                    }
//                    if (obj.resetpwd) {
//                        window.location = "http://safe.taotao.com/resetPwd/reset.action?username=" + obj.resetpwd;
//                        return;
//                    }
//                    $("#loginsubmit").removeAttr("disabled");
//                    verc();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result) {
                    ...
                    if(obj.venture){                    	
                    	window.parent.location = "http://safe.taotao.com/dangerousVerify/index.action?username=" + obj.venture + "&ReturnUrl="+encodeURI(window.parent.location) + "&t=" + new Date().getTime();
                    	return;
                    }
                    if (obj.resetpwd) {
                        window.parent.location = "http://safe.taotao.com/resetPwd/reset.action?username=" + obj.resetpwd;
                        return;
                    }
                    $("#loginsubmitframe").removeAttr("disabled");
                    verc();
...
function showBox() {
    ...
    var dwl = "<a href=\"http://taotao.com/securityctl/"+pgeditor.getLinkHtml()+"\">立即下载</a>";
    jQuery.jdThickBox({
        type:"text",
        title:"提示",
        width:350,
        height:130,
        source:"<div class=\"step step-0\"><div class=\"mc\"><p>安全控件可提高账户安全性，加密保护您的密码。<a href=\"http://help.taotao.com/help/question-61.html\" style=\"color:#005EA7;cursor:pointer\" target=\"_blank\">了解更多&gt;</a></p><ul><li>立即下载</li><li>手动安装</li><li>刷新页面，继续购物</li></ul><div class=\"btn\">"+dwl+"</div></div></div>",
        _autoReposi:true
    });
}
