...
function showWhiteBar() {
	try {
		jQuery.getJSON("http://baitiao.taotao.com/account/query?callback=?", function(data) {
			if (data == null) {
				return;
			}
...
function addGiftCard(obj, giftCardType) {
	if ($("#lpkKeyPressFirst" + "-" + giftCardType).val() == "" || $("#lpkKeyPressSecond" + "-" + giftCardType).val() == ""
		...
		if (giftCardType == 3) {
			showMessageWarn("请输入淘淘E卡密码");
		} else {
			showMessageWarn("请输入淘淘卡密码");
		}
		return;
	}
...
//					successUrl = "http://order.taotao.com/center/list.action";
//					// 等待拆单，定时450毫秒
//					window.setTimeout('window.location.href=successUrl+"?rd="+Math.random();', 450);
//					return;
//
//				} else {
//					successUrl = "http://s.trade.taotao.com/success/success.action";
...
//								tmpHtml = tmpHtml + "<a target=\"_parent\" href=\"http://item.taotao.com/" + skuList[i] + ".html\">" + skuList[i] + "</a>,";
//							}
...
function checkShowOpenPwd(showOpenPayPwd, existsJdbeanPromotion, checkJdbeanPromotion) {
	if (existsJdbeanPromotion == true) {
		if (showOpenPayPwd == false) {
			...
			$("#submit_message").html(
					"<span>为保障您的账户资金安全，京豆暂时不可用，请先<a href='http://safe.taotao.com/user/paymentpassword/safetyCenter.action' target='_blank'>开启支付密码 </a></span> ").show();
		}
	}
}
