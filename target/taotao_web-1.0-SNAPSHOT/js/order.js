...
var cartUrl = "http://cart.taotao.com/cart/cart.html";
var lipinkaPhysicalUrl = "http://market.taotao.com/giftcard/#entity";
...
function loadAreaDetail(id, provinceId, cityId, countyId, townId) {
	...
	jQuery.ajax( {
		...
		$("#codHelpUrl").attr("href","http://help.taotao.com/help/distribution-768-"+provinceId+"-"+cityId+"-"+countyId+"-"+(isEmpty(townId)?0:townId)+"-"+(new Date()).getTime()+".html");
	}
},
error : function(XMLHttpResponse) {
	alert("系统繁忙，请稍后再试！");
...
function loadCitys() {
	...
	$("#codHelpUrl").attr("href","http://help.taotao.com/help/distribution-768-"+(isEmpty(provinceId)?0:provinceId)+"-"+0+"-"+0+"-"+0+"-"+(new Date()).getTime()+".html");
...
function loadCountys() {
	...
	$("#codHelpUrl").attr("href","http://help.taotao.com/help/distribution-768-"+$("#consignee_province").find("option:selected").val()+"-"+(isEmpty(cityId)?0:cityId)+"-"+0+"-"+0+"-"+(new Date()).getTime()+".html");
...
function loadTowns() {
	...
	$("#codHelpUrl").attr("href","http://help.taotao.com/help/distribution-768-"+$("#consignee_province").find("option:selected").val()+"-"+$("#consignee_city").find("option:selected").val()+"-"+(isEmpty(countyId)?0:countyId)+"-"+0+"-"+(new Date()).getTime()+".html");
...
function use_NewConsignee() {
	...
	$("#span_town").hide();
	$("#areaNameTxt").text("");
	$("#codHelpUrl").attr("href","http://help.taotao.com/help/distribution-768-0-0-0-0-"+(new Date()).getTime()+".html");
	// #高亮选中
...
function showWhiteBar(){
	try{
		$.getJSON("http://baitiao.taotao.com/account/query?callback=?",
				function(data){
					if(data==null){
						return;
					}
...
function useOrCancelGiftCard(url, key, obj, checked, bindFlag, giftCardType) {
	...
	var giftCardProxyId = giftCardId;
	var orderGiftCardProxyItem = orderGiftCardItem;
	var giftCardTypeName = "淘淘卡";
	if(giftCardType == 3){
		giftCardProxyId = giftECardId;
		orderGiftCardProxyItem = orderGiftECardItem;
		orderGiftCardModule = OrderAppConfig.Module_GiftECard;
		giftCardTypeName = "淘淘E卡";
	}
	param = addFlowTypeParam(param);
	jQuery.ajax( {
		type : "POST",
...
//		});
   ...
   error : function(XMLHttpResponse) {
	...
	function getCheckCodeTemplate(encryptClientInfo) {
		...
		var checkCodeUrl="";
		var checkCodeUrl="http://captcha.taotao.com/verify/image?acid="+rid+"&srcid=trackWeb&is="+encryptClientInfo;
	        ...
	        success: function(dataResult) {
				if(dataResult.success){
					...
					if(errorMessage.indexOf("已注册")>-1){
						errorMessage=errorMessage+"，<a href='https://passport.taotao.com/new/login.aspx?ReturnUrl=http%3A%2F%2Fcart.taotao.com%2Fcart%2Fcart.html' >立即登录</a>";
					}
	...
	function loadGiftBuyHidePrice(){
		if(isGiftBuy()){
			cartUrl ="http://cart.gift.taotao.com/cart/cart.html";
			$("#cartRetureUrl").attr("href","http://cart.gift.taotao.com/cart/cart.html");
			$("#cartRetureUrl").text("返回修改礼品购购物车");
			$("#giftBuyHidePriceDiv").show();
			$("#consigneeTitleDiv").text("收礼人信息");
		}else{
			cartUrl ="http://cart.taotao.com/cart/cart.html";
			$("#cartRetureUrl").attr("href","http://cart.taotao.com/cart/cart.html");
	...
	jQuery.ajax({
		...
		success : function(dataResult, textStatus) {
			...
			if(dataResult=="null"){
				...
				window.location.href="http://trade.taotao.com/order/getOrderInfo.action?rid=" + Math.random();
			}
	...
	if(existsJdbeanPromotion == true) {
		if(showOpenPayPwd==false){
			...
			if(checkJdbeanPromotion==false){
				$("#submit_message").html("<span>京豆不足,不能使用京豆优惠购,点击<a href='http://cart.taotao.com/cart/cart.html?outBean=1'>返回购物车 </a></span> ").show();
			}
		}else{
			$("#submit_message").html("<span>为保障您的账户资金安全，京豆暂时不可用，请先<a href='http://safe.taotao.com/user/paymentpassword/safetyCenter.action' target='_blank'>开启支付密码 </a></span> ").show();
		}
	}
}
...
function shipmentToggle(th){
	if($(th).attr("id")=="jd-shipment"){// 选择淘淘配送
	...
	}else if($(th).attr("id")=="other-shipment"){// 选择淘淘三方配送
		$(th).parent().parent().addClass("item-selected");
		$("#pick-shipment").parent().parent().removeClass("item-selected");
