...
function search(a) {
    ...
    switch (f) {
        ...
        case 5:
            ...
            b = "http://search.e.taotao.com/searchDigitalBook?ajaxSearch=0&enc=utf-8&key={keyword}&page=1{additional}";
            break;
        case 6:
            i = "-1", b = "http://music.taotao.com/8_0_desc_0_0_1_15.html?key={keyword}"
    }
...
function getRandomDomain(a) {
    ...
    return "http://imgtaotao.com/".replace("{0}", b)
}
function setWebBILinkCount(a) {
    try {
        if (a.length > 0) {
            var b = document.createElement("script");
            b.type = "text/javascript", b.src = "http://counter.taotao.com/aclk.aspx?key=" + a, document.getElementsByTagName("head")[0].appendChild(b)
        }
    } catch (c) {
    }
}
...
function openCompare() {
    switch (proIDs.length) {
        case 1:
            ...
            break;
        case 2:
            window.open("http://www.taotao.com/pcompare.aspx?s1=" + proIDs[0] + "&s2=" + proIDs[1]);
            break;
        case 3:
            window.open("http://www.taotao.com/pcompare.aspx?s1=" + proIDs[0] + "&s2=" + proIDs[1] + "&s3=" + proIDs[2]);
            break;
        case 4:
            window.open("http://www.taotao.com/pcompare.aspx?s1=" + proIDs[0] + "&s2=" + proIDs[1] + "&s3=" + proIDs[2] + "&s4=" + proIDs[3]);
            break;
        default:
    ...
    for (var b = a.getElementsByTagName("img"), c = 0; c < b.length; c++)
        b[c].onerror = function() {
            ...
            if (b) {
                ...
                this.src = "http://taotao.com/lib/img/e/blank.gif", this.className = a
            }
        }
...
var loguri = "http://csc.taotao.com/log.ashx?type1=$type1$&type2=$type2$&data=$data$&pin=$pin$&referrer=$referrer$&callback=?";
callback1 = function() {
}, log = function(a, b) {
    var c = "";
    ...
    var e = ("https:" == document.location.protocol ? "https://mercuryssl" : "http://mercury") + ".taotao.com/log.gif" + "?t=other.000000" + "&m=UA-J2011-1" + "&v=" + encodeURIComponent("t1=" + a + "$t2=" + b + "$p0=" + c) + "&ref=" + encodeURIComponent(document.referrer) + "&rm=" + (new Date).getTime(), f = new Image(1, 1);
    f.src = e
}, mark = function(a, b) {
    log(1, b, a)
